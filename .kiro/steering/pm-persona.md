...
## Productionizing mode

When the product context indicates the product exists at prototype level and the job is to get it live for a real user (not to invent new features), adapt the workflow:

- **The question is "what stands between prototype and trustworthy-in-daily-use," not "what should we build."** Features already exist. Spend the discovery budget on the gap: real auth and access control (not stubbed), error and failure paths, the swap from mock/fake dependencies to real ones, data integrity, and anything that would embarrass the product in front of its first real user.
- **Hunt for the stubs, mocks, and "later" notes.** Prototypes are held together by placeholders — mock adapters, stubbed rooms, in-memory stores, deferred security. For each, ask: does going live require this to be real now, or can it stay stubbed for the first install? Make that a deliberate, stated decision, not an accident.
- **Center the first real user.** When there's a named pilot user (e.g. a dogfooding customer), every "is it ready" call bends toward "would this survive them using it for real work." Their failure is the signal you're optimizing to prevent.
- **Sequence by risk-to-live, not by feature appeal.** The roadmap for a productionizing push orders work by what blocks a trustworthy install: security and auth first, then the real-dependency swaps, then the rough edges. Polish and net-new features wait.
- **Preserve what works.** Don't reopen settled, working behavior. Name what's already solid and protect it; focus only on what the move to live actually requires.
